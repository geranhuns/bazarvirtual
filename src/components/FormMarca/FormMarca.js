...
function FormMarca() {
    ...
    const renderIconoRedSocial = (redSocial) => {
        switch (redSocial) {
            case 'facebook':
                return <i className="fab fa-facebook"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6"><path d="M12 2C6.48 2 2 6.48 2 12C2 16.42 5.13 20.13 9.5 20.88V14.59H7.81V12H9.5V9.81C9.5 7.5 10.88 6.5 12.85 6.5H14.19V9H12.85C12.55 9 12.5 9.23 12.5 9.5V12H14.19L13.88 14.59H12.5V20.88C16.87 20.13 20 16.42 20 12C20 6.48 15.52 2 12 2Z" /></svg></i>
            case 'twitter':
    ...
    return (
        <form className="w-[50%] mx-auto mt-10" onSubmit={(e) => e.preventDefault()}>
            <div className="flex flex-col gap-4 pb-4">
                <div className="flex flex-col gap-2">
                    <label htmlFor="logoMarca" className='font-semibold text-raw-sienna-900'>Logo de la marca</label>
                    <input type="file" id="logoMarca" className="border border-raw-sienna-300 p-4" onChange={handleImagen} />
                ...
                <div className="flex flex-col gap-2">
                    <label htmlFor="nombreMarca" className='font-semibold text-raw-sienna-900'>Nombre de la marca</label>
                    <input type="text" id="nombreMarca" className="border border-gray-300 p-2" />
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="sloganMarca" className='font-semibold text-raw-sienna-900'>Slogan de la marca</label>
                    <input type="text" id="sloganMarca" className="border border-gray-300 p-2" />
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="descripcionMarca" className='font-semibold text-raw-sienna-900'>Descripción de la marca</label>
                    <textarea id="descripcionMarca" className="border border-gray-300 p-2 w-full h-32 resize-none overflow-y-auto" />
                </div>
                <div className="flex flex-col gap-2">
                    <label htmlFor="redesSociales" className='font-semibold text-raw-sienna-900'>Redes sociales</label>
                    <div className="flex items-center">
                        <input type="text" id="redesSociales" className="border border-gray-300 p-2" value={redSocial} onChange={handleRedSocialChange} onKeyDown={handleRedSocialKeyDown} />
                        {mensajeError && <p className="text-red-500 text-sm font-semibold ml-2">{mensajeError}</p>}
                    </div>
                ...
                <button className="bg-raw-sienna-500 text-white p-2 rounded-md w-full self-center">Guardar</button>
            </div>
        </form>
    )
}
