    ...
    return (
        <>
            <div className="fixed inset-0 z-50 bg-gray-600/80 w-full h-dvh backdrop-blur-md flex flex-col mt-16">
                ...
                <div className="bg-customGreen fixed w-7/12 px-10 pb-10 flex flex-col items-start justify-center left-20 max-sm:w-11/12 max-sm:left-1 ">
                    <button className="bg-raw-sienna-500 w-10 h-10 flex justify-center items-center self-end rounded-sm" onClick={() => setOpen(!open)} ><MdClose className="w-full h-full" /></button>
                    <form onSubmit={handleSubmit(onSubmit)} className="bg-form-newDate-green w-full h-5/6 flex flex-col p-10 items-center text-customGreen text-2xl ">
                        <label htmlFor="">Lugar</label>
                        <input className="rounded-sm" type="text" {...register("Lugar")} />
