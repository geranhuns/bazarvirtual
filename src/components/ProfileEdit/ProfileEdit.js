...
export default function ProfileEdit({ userData, setIsSubmit, isSubmit }) {
  ...
  const onSubmit = async  (data) =>{
    ...
    let password
    data.password && data.passwordCheck != data.password? password="" : password= data.password;
    ...
    }else{console.log("data adjust esta vacio")}
        ...
        <div className=" flex flex-col gap-3 w-96 mb-3">
         <div className=" w-full h-2/6   p-15 flex items-center max-sm:rounded-lg">
              <div className=" w-2/3 h-2/3 mx-auto rounded-full relative  ">
                <img className="w-full h-full rounded-full object-cover object-center" src={userData.profilePicture} alt="" />
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              {...register('passwordCheck', {
                required: passwordValue ? 'Confirmar contraseña es obligatorio cuando se proporciona una contraseña' : false,
                minLength: {
                  value: 8,
                  message: 'La confirmación de contraseña debe tener al menos 8 caracteres'
                },
                validate: value =>
                  passwordValue && value !== passwordValue ? 'Las contraseñas no coinciden' : true
              })}
            />
              {errors.passwordCheck && <p className="text-red-500">{errors.passwordCheck.message}</p>}
          </div>
          <button type="Submit" className="flex items-center justify-center p-1 rounded-lg text-lg font-medium h-9 bg-raw-sienna-500 text-raw-sienna-50">Guardar cambios</button>
